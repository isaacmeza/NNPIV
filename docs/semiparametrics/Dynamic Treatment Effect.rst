...
For the uncentered path-specific score :math:`\psi_d`, the finite-bandwidth target is
...
Writing :math:`\ell_{\lambda,v}=K/\mathbb{E}[K]`, its centered score contribution is :math:`\ell_{\lambda,v}\{\psi_d-\theta_{d,\lambda}(v)\}`.
...
OR and IPW use the same centering; see :ref:`localized-ratio-targets` for its inferential interpretation.
