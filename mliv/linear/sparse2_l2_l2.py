...
class sparse2_l2vsl2(_SparseLinear2AdversarialGMM):
    ...
    def _check_duality_gap(self, A, B, C, D, Y):
        self.max_response_loss_ = np.linalg.norm(np.mean(D * (Y - np.dot(A, self.alpha_)).reshape(-1, 1), axis=0), ord=2)\
            + np.linalg.norm(np.mean(C * (np.dot(A, self.alpha_) - np.dot(B, self.beta_)).reshape(-1, 1), axis=0), ord=2)\
    ...
    def fit(self, A, B, C, D, Y):
        ...
        while t < T:
            ...
            if d_a * d_d < n**2:
                ...
                cors1[:] = - np.mean(test_fn * A, axis=0) + mu * alpha
            if d_a * d_c < n**2:
                cors1[:] += ac @ w2
            else:
                test_fn = np.dot(C, w2).reshape(-1, 1)
                cors1[:] += np.mean(test_fn * A, axis=0)
            ...
            if d_b * d_c < n**2:
                ...
                test_fn = np.dot(C, w2).reshape(-1, 1)
                cors2[:] = - np.mean(test_fn * B, axis=0) + mu * beta
            ...
            if d_a * d_d < n**2:
                ...
                res1[:] = yd - np.mean(D * pred_fn, axis=0) 
    ...
    def fit(self, A, B, C, D, Y):
        ...
        while t < T:
            ...
            if d_a * d_d < n**2:
                ...
                test_fn = np.dot(D, w1).reshape(-1, 1)
                cors1[:] = - np.mean(test_fn * A, axis=0) + mu * aa @ alpha
            if d_a * d_c < n**2:
                cors1[:] += ac @ w2
            else:
                test_fn = np.dot(C, w2).reshape(-1, 1)
            ...
            if d_b * d_c < n**2:
                ...
                test_fn = np.dot(C, w2).reshape(-1, 1)
                cors2[:] = - np.mean(test_fn * B, axis=0) + mu * bb @ beta
