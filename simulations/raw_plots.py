...
def plot_raw_ind(param_estimates, metric_results, config):
    for dgp_name, mdgp in metric_results.items():
        metric_name = 'raw'
        for it, method_name in enumerate(mdgp.keys()):
            if config['dgp_opts']['fn'] in config['subplots']['fn_plot'] :
                ...
                plt.legend()
                plt.tight_layout()
                plt.savefig(os.path.join(config['target_dir'], 'raw_{}_dgp_{}_{}_{}.png'.format(
                    filesafe(metric_name), dgp_name, method_name, config['dgp_opts']['fn'])), dpi=300)
                plt.close()
    return
