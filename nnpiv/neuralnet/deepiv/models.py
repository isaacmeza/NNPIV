...
class Treatment(Model):
    ...
    def compile(self, optimizer, loss, metrics=None, loss_weights=None,
        ...
        if loss == "mixture_of_gaussians":
            ...
            def loss(y_true, y_pred): return densities.mixture_of_gaussian_loss(y_true,
                                                                                y_pred,
                                                                                n_components)
...
class Response(Model):
    ...
    def compile(self, optimizer, loss, metrics=None, loss_weights=None, sample_weight_mode=None,
        ...
        if unbiased_gradient:
            if loss in ["MSE", "mse", "mean_squared_error"]:
                ...
                replace_gradients_mse(
                    self, optimizer, batch_size=batch_size, n_samples=n_samples)
            else:
    ...
    def fit(self, x=None, y=None, batch_size=512, epochs=1, verbose=1, callbacks=None,
        ...
        if observed_treatments is None:
            generator = SampledSequence(
                x[1:], x[0], y, batch_size, self.treatment.sample, samples_per_batch)
        else:
    ...
    def expected_representation(self, x, z, n_samples=100, batch_size=None, seed=None):
        ...
        if not hasattr(self, "_E_representation"):
            ...
            def pred(inputs, n_samples=100, seed=None):
                ...
                batch_features = [features.repeat(
                    n_samples, axis=0)] + [samples]
                representation = intermediate_layer_model.predict(
                    batch_features)
    ...
    def dropout_predict(self, x, z, n_samples=100):
        ...
        if not hasattr(self, "_dropout_predict"):
            ...
            def pred(inputs, n_samples=100):
                ...
                samples = self.treatment.sample(
                    inputs, n_samples, use_dropout=True)
                # prepare inputs for the response network
                rep_inputs = [i.repeat(n_samples, axis=0)
                              for i in inputs[1:]] + [samples]
    ...
    def credible_interval(self, x, z, n_samples=100, p=0.95):
        ...
        if isinstance(x, list):
            ...
            n = x.shape[0]
        alpha = (1 - p) / 2.
        samples = self.dropout_predict(
            x, z, n_samples).reshape((n, n_samples, -1))
    ...
    def fit_confidence_interval(self, x_lo, z_lo, p_lo, y_lo, n_samples=100, alpha=0.):
        ...
        eta_lo_prime = pca.transform(
            self.conditional_representation(x_lo, p_lo))
        ...
        def pred(xx, pp):
            H = self._add_constant(pca.transform(
                self.conditional_representation(xx, pp)))
...
class SampledSequence(keras.utils.Sequence):
    ...
    def __getitem__(self, idx):
        ...
        features = [
            inp[idx * self.batch_size:(idx + 1) * self.batch_size, :] for inp in self.features]
        sampler_input = instruments + features
        samples = self.sampler(sampler_input, self.n_samples)
        batch_features = [f[idx * self.batch_size:(idx + 1) * self.batch_size].repeat(
            self.n_samples, axis=0) for f in self.features] + [samples]
        batch_y = self.outputs[idx * self.batch_size:(
            idx + 1) * self.batch_size].repeat(self.n_samples, axis=0)
        if idx == (len(self) - 1):
            self.shuffle()
...
class OnesidedUnbaised(SampledSequence):
    ...
    def __getitem__(self, idx):
        ...
        features = [
            inp[idx * self.batch_size:(idx + 1) * self.batch_size, :] for inp in self.features]
        observed_treatments = self.treatments[idx *
                                              self.batch_size:(idx + 1) * self.batch_size, :]
        sampler_input = instruments + features
